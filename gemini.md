...
- **Command: `baca error`**
  ...
  - **Aturan Eksekusi Makro (Alur Sapu Bersih Tanpa Kompromi):**
    1. **Global App Auditing & Pencatatan Issue (STRICT):** AI dilarang keras hanya berfokus pada satu file atau satu pesan error yang dikirimkan user. AI wajib menggunakan tool filesystem secara masif untuk memetakan seluruh file routing, mendata semua halaman fisik yang aktif, serta menguji seluruh alur logika fitur (Auth, Form CRUD, Captcha, Validation Engine) yang ada di dalam repositori untuk berburu silent error atau celah visual.
       * **Pembuatan Cetak Biru Issue (`/.docs/issues.md`):** Segera setelah pemindaian selesai, AI **MUTLAK WAJIB** membuat berkas dokumentasi `/.docs/issues.md` yang memetakan secara detail daftar temuan bug, path file yang bermasalah, dampak kerusakan, dan hipotesis rencana tindakan perbaikan.
    ...
    5. **Looping Guard (Proteksi Loop Tanpa Batas):**
...
3. **Pencatatan Dokumentasi `issues.md` (`/.docs/issues.md`):** AI wajib merangkum seluruh hasil pemindaian dan temuan kesalahan ke dalam berkas `/.docs/issues.md` (mencakup ID issue, file path, deskripsi error, analisis penyebab, dan rencana tindakan perbaikan).
...
7. **Looping Guard (Proteksi Loop Tanpa Batas):** Percobaan perbaikan pada satu titik error dibatasi maksimal **3 kali percobaan berturut-turut**. Jika tetap gagal, AI wajib menghentikan loop, melakukan git restore/checkout ke commit bersih terakhir, memperbarui berkas `issues.md`, dan melaporkan statusnya secara transparan ke pengguna.
